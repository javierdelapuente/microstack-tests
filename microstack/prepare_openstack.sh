...
lxc init ubuntu:24.04 openstack --vm -c limits.cpu=12 -c limits.memory=${RAM_MEMORY} -d root,size=${ROOT_DISK_SIZE} --config=user.network-config="$(cat ./openstack-network-config)" --config=user.user-data="$(cat ./openstack-user-data | envsubst '$RISK,$ENABLE_CEPH' )"
